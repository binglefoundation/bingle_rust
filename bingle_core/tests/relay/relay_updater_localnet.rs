...
fn relay_updater_localnet_e2e_matrix() {
    ...
    let admin_ops = test_util::ops_from_mnemonic(
        test_util::ADDRESS_APP_ADMIN,
        test_util::PASSPHRASE_APP_ADMIN,
        cfg.clone(),
    );
    let relay1_ops = test_util::ops_from_mnemonic(
        test_util::ADDRESS_SPEND,
        test_util::PASSPHRASE_SPEND,
        cfg.clone(),
    );
    let relay2_ops = test_util::ops_from_mnemonic(
    ...
    let (app_id, asset_id) =
        test_util::deploy_bingle_app_and_asset(&relay1_ops, "BINGLE$", 1_000_000);
    let creator_ab = AlgoBingle::new(admin_ops.clone(), app_id, 0);

    let relay1_addr = SocketAddr::new(
        IpAddr::V4(Ipv4Addr::LOCALHOST),
        test_util::find_unused_loopback_port(),
    );
    let relay2_addr = SocketAddr::new(
        IpAddr::V4(Ipv4Addr::LOCALHOST),
        test_util::find_unused_loopback_port(),
    );
    ...
    let relay1_root = RelayInfo::root(test_util::get_signed_advert_record(
        &relay1_id,
        test_util::PASSPHRASE_SPEND,
        relay1_addr,
        true,
    ...
    let relay2_root = RelayInfo::root(test_util::get_signed_advert_record(
        &relay2_id,
        test_util::PASSPHRASE_RECEIVE,
        relay2_addr,
        true,
...
fn run_scenario(
    ...
    let init_cache = updater
        .relay_info_cache()
        .list_all_relays(my_id.as_str(), false);
    assert_eq!(
        init_cache.len(),
    ...
    let expected_root_ids: BTreeSet<String> = registered_roots
        .iter()
        .map(|relay| relay.id().to_string())
        .collect();
    let init_root_ids: BTreeSet<String> = init_cache
        .iter()
        .map(|relay| relay.id().to_string())
        .collect();
    ...
    if !expect_selected {
        assert!(
            selected.is_none(),
            "{scenario_name}: expected no relay selected"
        );
        return;
    }
    ...
    assert!(
        selected.is_some(),
        "{scenario_name}: expected selected relay"
    );
    ...
    let all_relays = updater
        .relay_info_cache()
        .list_all_relays(my_id.as_str(), true);
    ...
    assert!(
        non_root_count >= min_expected_non_root_count,
        "{scenario_name}: expected at least {min_expected_non_root_count} non-root relays"
    );

    let selected_in_cache = all_relays
        .iter()
        .find(|relay| relay.id() == selected_relay.id());
    assert!(
...
fn register_relay_static_endpoint(
    ...
    relay_ops
        .opt_in_app(app_id)
        .expect("relay opt-in should succeed");
    creator_ab
