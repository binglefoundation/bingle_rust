...
use bingle_core::ddb::client::DdbClient;
use bingle_core::relay::relay_client::RelayClient;
...
impl DdbClient for DdbMock {
    ...
    fn lookup(
        &self,
        _id: &str,
    ) -> Result<NetworkEndpoint, bingle_core::api::bingle_api::BingleError> {
        self.lookup_result
            .clone()
            .ok_or_else(|| bingle_core::api::bingle_api::BingleError::Other("no lookup".into()))
    }
    fn start_load_from_peer(
        &self,
        _peer_id: &str,
    ) -> Result<usize, bingle_core::api::bingle_api::BingleError> {
        Err(bingle_core::api::bingle_api::BingleError::Other(
            "not used".into(),
        ))
    }
    fn signoff(&self) -> Result<(), bingle_core::api::bingle_api::BingleError> {
        Err(bingle_core::api::bingle_api::BingleError::Other(
            "not used".into(),
        ))
    }
