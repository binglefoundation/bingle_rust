...
use bingle_core::messages::types::{RelayCall, RelayListen, RelayListenResponse};
use bingle_core::messages::{Message, RelayMessage};
use bingle_core::turn::turn_handler::TurnClientImpl;
...
pub fn bingle_api_send_via_relay() {
    ...
    let client_turn_handler: std::sync::Arc<
        dyn Fn(&dyn NetworkMux, &SocketAddr, &[u8]) + Send + Sync,
    > = Arc::new(
        move |source: &dyn NetworkMux, from: &SocketAddr, packet: &[u8]| {
                ...
                turn_client_clone.handle_turn_incoming(Some(from), Some(b_addr), packet)
            {
                ...
                if let Some(udp) = source.as_any().downcast_ref::<UdpNetworkMux>() {
                    ...
                    tracing::warn!("source is not UdpNetworkMux; cannot reprocess");
                }
            } else {
    ...
    let mut server = DtlsOpenSsl::new("server".to_string())
        ...
        .with_handle_message(Arc::new(
            move |server: &dyn Dtls, from: &NetworkEndpoint, _issuer: &str, data: &[u8]| {
    ...
    let a_addr_capture = Arc::new(Mutex::new(None));
    {
        ...
        let th: bingle_core::dtls::network_mux_trait::HandleTurn = Arc::new(
            move |source: &dyn NetworkMux, from: &SocketAddr, packet: &[u8]| {
        ...
        let dh: bingle_core::dtls::network_mux_trait::HandleDtls = Arc::new(
            move |source: &dyn NetworkMux, from: &NetworkEndpoint, packet: &[u8]| {
    ...
    let resp = Message::Relay(RelayMessage::ListenResponse(RelayListenResponse {
        app: None,
        response_tag: None,
    }));
    ...
    mux_relay
        .write(&b_nsk, &resp_bytes)
        .expect("relay write to B");
    ...
    let turn_client_a = api
        .access_unsafe_for_tests(|a: &mut BingleApiImpl| a.engine_turn_client_handler_for_tests());
    use bingle_core::turn::turn_handler::TurnHandler;
    ...
    for _ in 0..10 {
        if api
            .access_unsafe_for_tests(|a: &mut BingleApiImpl| {
                a.send_message_to_network(&nsk, &uid, payload.clone(), None)
            })
            .unwrap_or(false)
        {
            ok = true;
    ...
    assert!(
        delivered,
        "expected destination to receive data via DTLS over relay"
    );
