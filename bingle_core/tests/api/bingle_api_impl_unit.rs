...
impl Dtls for MockDtls {
    fn start(&mut self, _mux: Arc<bingle_core::dtls::UdpNetworkMux>) -> Result<()> {
        Ok(())
    }
...
pub fn unit_send_message_to_network_calls_dtls_send() {
    ...
    let ok = api
        .access_unsafe_for_tests(|a: &mut BingleApiImpl| {
            a.send_message_to_network(
                ...
                &uid,
                msg.clone(),
                Some(Arc::new(move |p, s| {
                    progress_calls_closure.lock().unwrap().push((p, s));
                })),
            )
        })
        .unwrap();
    assert!(
        ok,
        "send_message_to_network should return true on MockDtls::send Ok"
    );
...
pub fn start_sets_issuer_and_passes_to_dtls_send() {
    ...
    impl Dtls for MockDtlsCapture {
        ...
        fn send(
            ...
            let addr = to
                .inet_socket_address()
                .expect("MockDtlsCapture::send requires inet_socket_address");
    ...
    let sk: [u8; 32] = [7u8; 32];
    let addr = byte_key_to_address(
        &ed25519_dalek::SigningKey::from_bytes(&sk)
            .verifying_key()
            .to_bytes(),
    )
    .expect("addr");
    let issuer_expected = format!("{}{}", addr, bingle_core::protocol::ISSUER_SUFFIX);
    ...
    api.access_unsafe_for_tests(|a: &mut BingleApiImpl| {
        a.set_issuer_for_tests(issuer_expected.clone())
    });
    ...
    let ok = api
        .access_unsafe_for_tests(|a: &mut BingleApiImpl| {
            a.send_message_to_network(&nsk, &uid2, serde_json::json!({"k": 1}), None)
        })
        .unwrap();
    assert!(ok);
