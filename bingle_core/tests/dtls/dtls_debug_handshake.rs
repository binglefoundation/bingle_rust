...
pub fn dtls_debug_parses_handshake_type_and_extensions() {
    use bingle_core::dtls::dtls_debug::{dtls_udp_to_json_with_level, json_to_dtls_udp};
    ...
    let json =
        dtls_udp_to_json_with_level(&datagram, tracing::Level::TRACE).expect("dtls_udp_to_json ok");
    ...
    let ht_name = hs
        .get("handshake_type_name")
        .and_then(|x| x.as_str())
        .unwrap_or("");
    assert!(
        ht_name.eq_ignore_ascii_case("client_hello"),
        "unexpected handshake_type_name: {}",
        ht_name
    );
    ...
    let mut have_sn = false;
    let mut have_ks = false;
    for e in exts_json {
        ...
        if id == 0x33 {
            have_ks = true;
            assert_eq!(name, "key_share");
        }
    }
    assert!(
        have_sn && have_ks,
        "expected server_name and key_share extensions"
    );
