use bingle_core::util::config_utils::parse_stun_list;
...
fn parse_stun_list_takes_first_ipv4() {
    ...
    assert_eq!(
        result[0],
        SocketAddr::new(Ipv4Addr::new(127, 0, 0, 1).into(), 3478)
    );
}
...
fn parse_stun_list_rejects_direct_ipv6() {
    ...
    assert!(
        result.is_err(),
        "Should reject IPv6 address when it's the only entry"
    );
}
...
fn parse_stun_list_dns_ipv4_ipv6() {
    ...
    for addr in result {
        assert!(
            addr.is_ipv4(),
            "All resolved addresses should be IPv4, found {:?}",
            addr
        );
    }
}
