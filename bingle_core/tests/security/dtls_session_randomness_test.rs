...
use bingle_core::api::bingle_api::{BingleApi, Handle, NetworkEndpoint, StartOptions};
...
fn dtls_session_randomness_test() {
    ...
    for s in 0..session_count {
        ...
        assert!(
            ...
            "Failed to send message in session {}",
            s
        );
        ...
        let app_data = packets
            .iter()
            .find(|p| !p.is_empty() && p[0] == 23)
            .expect(&format!(
                "Should have captured Application Data packet in session {}",
                s
            ));
    ...
    // Validation
    assert_eq!(
        session_ciphertexts.len(),
        session_count,
        "Did not capture enough session ciphertexts"
    );

    for i in 0..session_count {
        for j in i + 1..session_count {
            assert!(
                session_ciphertexts[i] != session_ciphertexts[j],
                "Identical ciphertexts in session {} and {}!",
                i,
                j
            );
            ...
            if i < session_client_randoms.len() && j < session_client_randoms.len() {
                assert!(
                    session_client_randoms[i] != session_client_randoms[j],
                    "Identical ClientHello randoms in session {} and {}!",
                    i,
                    j
                );
            }
            ...
            if i < session_server_randoms.len() && j < session_server_randoms.len() {
                assert!(
                    session_server_randoms[i] != session_server_randoms[j],
                    "Identical ServerHello randoms in session {} and {}!",
                    i,
                    j
                );
            }
    ...
    println!(
        "Cross-session randomness test passed! All {} sessions had unique ciphertexts.",
        session_count
    );
    if !session_client_randoms.is_empty() {
        println!(
            "Verified {} unique ClientHello randoms.",
            session_client_randoms.len()
        );
    }
    if !session_server_randoms.is_empty() {
        println!(
            "Verified {} unique ServerHello randoms.",
            session_server_randoms.len()
        );
    }
}
