...
fn make_ee_rsa(
    ...
    builder
        .sign(ca_key, openssl::hash::MessageDigest::null())
        .expect("sign");
    (builder.build(), pkey)
}
...
fn dh_1024_rejected() {
    ...
    acceptor_builder
        .set_cipher_list("DHE-RSA-AES128-GCM-SHA256")
        .unwrap();
    ...
    client_api
        .access_unsafe_for_tests(|a| a.start(&client_opts))
        .expect("start client api");

    std::thread::sleep(Duration::from_millis(100));

    // 3) Attempt send to server
    let nsk = bingle_core::api::bingle_api::NetworkEndpoint::new_direct(server_addr);
    let ok = client_api.send_message_to_network(
        &nsk,
    ...
    assert!(
        !ok_val,
        "Handshake should have failed for 1024-bit DH parameters"
    );
...
fn rsa_1024_server_cert_rejected() {
    ...
    assert!(
        !ok_val,
        "Handshake should have failed for 1024-bit RSA server certificate"
    );
