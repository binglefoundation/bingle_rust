...
pub fn start_with_addr_notifies_listening_true() {
    ...
    eng.set_on_listening_handler(Some(Arc::new(
        move |listening, _nat_type: bingle_core::engine::NatType| {
            if listening {
    ...
    assert!(
        ...
        "OnListening handler should be called with true for static-ip start"
    );
    ...
    assert!(
        local.is_some(),
        "local_bind_addr_for_tests should return Some"
    );
}
