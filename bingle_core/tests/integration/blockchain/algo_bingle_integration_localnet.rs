...
pub fn bingle_buy_register() {
    ...
    let app_asa_before = creator
        .asset_holding(&app_addr, asset_id)
        .expect("app asa before");
    ...
    ab.buy_bingle(app_id, asset_id, 1)
        .expect("buy_bingle group call");

    let receiver_asa_after_buy = creator
        .asset_holding(ADDRESS_RECEIVE, asset_id)
        .expect("receiver asa after buy");
    let app_asa_after_buy = creator
        .asset_holding(&app_addr, asset_id)
        .expect("app asa after buy");
    let app_algo_after_buy = creator
        .microalgos_at(&app_addr)
        .expect("app algo after buy");

    assert_eq!(
        receiver_asa_after_buy, 1,
        "receiver should hold 1 Bingle$ after buy"
    );
    assert_eq!(
        app_asa_after_buy,
        app_asa_before - 1,
        "app should have 1 fewer Bingle$ after buy"
    );
    assert_eq!(
        app_algo_after_buy,
        app_algo_before + 1 - MIN_FEE,
        "app should net -999 µAlgo after buy (received 1, paid 1000 inner fee)"
    );

    // register: receiver pays 1 Bingle$ to app and sets handle
    let handle = "alice";
    ab.register(app_id, asset_id, handle, 1)
        .expect("register group call");

    let receiver_asa_after_reg = creator
        .asset_holding(ADDRESS_RECEIVE, asset_id)
        .expect("receiver asa after register");
    let app_asa_after_reg = creator
        .asset_holding(&app_addr, asset_id)
        .expect("app asa after register");

    assert_eq!(
        receiver_asa_after_reg, 0,
        "receiver should have 0 Bingle$ after paying registration fee"
    );
    assert_eq!(
        app_asa_after_reg,
        app_asa_after_buy + 1,
        "app should have 1 more Bingle$ after register"
    );

    // Verify local state contains the handle
    let lstate = creator
        .local_state_for_account(app_id, ADDRESS_RECEIVE)
        .expect("local state query")
        .expect("some local state");
    let mut found = false;
    for (k, v) in lstate {
        if k == "Handle" {
