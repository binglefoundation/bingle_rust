...
// A second general-purpose client account. Individual tests use this address under local names
// (e.g. ADDRESS_B in send_message_to_id_integration, CLIENT_RESTRICTED_ADDR in connection_tests);
// declared here so the shared deploy helper can fund it as part of the standard signing set.
#[allow(dead_code)]
...
///
/// Funds the full standard signing set (role accounts plus the general-purpose client accounts
/// SPEND / RECEIVE / 10MIL / CLIENT_B). Because ~every localnet test deploys through this helper,
/// funding the whole set here makes tests order-independent: a test that spends from a shared client
/// account no longer relies on some other test having funded it first (issue #235).
///
...
pub fn deploy_bingle_app_and_asset(
    ...
    crate::setup_localnet::ensure_localnet_accounts_funded(&cfg, &standard_signing_addresses())
