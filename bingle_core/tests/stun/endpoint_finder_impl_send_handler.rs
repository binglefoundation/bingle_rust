...
use bingle_core::stun::{StunEndpointFinder, StunEndpointFinderImpl};
...
pub fn impl_uses_send_packet_handler_instead_of_udp() {
    ...
    finder.set_send_packet_handler(Some(Arc::new(move |host: &str, port: u16, data: &[u8]| {
        calls_clone
            .lock()
            .unwrap()
            .push((host.to_string(), port, data.len()));
    })));
    ...
    assert!(
        recorded
            .iter()
            .any(|(h, p, _)| *h == s1.ip().to_string() && *p == s1.port()),
        "send_packet_handler was not called for server {}",
        s1
    );
    assert!(
        recorded
            .iter()
            .any(|(h, p, _)| *h == s2.ip().to_string() && *p == s2.port()),
        "send_packet_handler was not called for server {}",
        s2
    );
    ...
    assert!(
        recorded.iter().any(|(_, _, len)| *len >= 20),
        "expected at least one STUN-sized packet (>=20 bytes)"
    );
}
