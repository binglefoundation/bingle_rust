...
use bingle_core::api::bingle_api::{NetworkEndpoint, UserId};
use bingle_core::messages::handlers::DefaultPrintingHandler;
use bingle_core::messages::types::RelayCall;
...
pub fn relay_call_allocates_channel_and_maps_pair() {
    ...
    let captured: Arc<Mutex<Option<(NetworkEndpoint, String, serde_json::Value)>>> =
    ...
    let call = Message::Relay(RelayMessage::Call(RelayCall {
        app: None,
        called_id: "CALLEEID".to_string(),
        tag: None,
    }));
    let responses =
    ...
    let obj = responses
        .into_iter()
        .next()
        .expect("expected a RelayResponse");
    ...
    let sent = captured
        ...
        .unwrap()
        .clone()
        .expect("RelayCalled should be sent to callee");
    ...
    let mapped_dest = turn
        .lookup_addr_by_channel_for_tests(ch_u16)
        .expect("ch->addr");
    assert_eq!(mapped_dest, (caller, callee));
    let p2c = turn
        .lookup_channel_for_pair_for_tests(&caller, &callee)
        .expect("pair->ch");
