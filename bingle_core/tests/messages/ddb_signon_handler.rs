...
use bingle_core::ddb::InMemoryDdbBackend;
use bingle_core::messages::handlers::{DefaultPrintingHandler, FromStruct, MessageHandler};
use bingle_core::messages::types::{DdbMessage, DdbSignon, Message};
...
pub fn test_on_ddb_signon_sends_response() {
    ...
    let router = Arc::new(bingle_core::messages::router::Router::new(api_weak.clone()));

    let sender_addr = SocketAddr::new(IpAddr::V4(Ipv4Addr::new(1, 2, 3, 4)), 1234);
    let from = FromStruct::new(
        "NEWNODE".to_string() + bingle_core::protocol::ISSUER_SUFFIX,
    ...
    let resp_msg =
        bingle_core::messages::marshal::from_json_value(resp_json).expect("valid message");
