...
use bingle_core::api::bingle_api::{
    BingleApi, BingleApiInternal, Handle, NetworkEndpoint, OnConnectHandler, OnMessageHandler,
    ProgressCallback, StartOptions, UserId,
};
use bingle_core::ddb::{AdvertRecord, DdbBackend, InetSocketAddress};
...
pub fn ddb_get_relays_status_returns_response_when_relay_available() {
    ...
    router.set_bingle_api(Some(crate::util::mock_bingle_api::to_weak(
        MockApiBoth::new_with_internal_override(Arc::new(InternalAvailable)),
    )));
    let backend = Arc::new(Mutex::new(bingle_core::ddb::InMemoryDdbBackend::new()));
    {
    ...
    let get = DdbGetRelaysStatus {
        app: "ddb".into(),
        epoch_id: -1,
        tag: Some("get_relays_status_tag".to_string()),
        text: None,
        data: None,
    };
    ...
    assert_eq!(
        out.get("app").and_then(|v: &serde_json::Value| v.as_str()),
        Some("ddb")
    );
    assert_eq!(
        out.get("type").and_then(|v: &serde_json::Value| v.as_str()),
        Some("relaysStatusResponse")
    );
    assert_eq!(
        out.get("responseTag")
            .and_then(|v: &serde_json::Value| v.as_str()),
        Some("get_relays_status_tag")
    );
    assert_eq!(
        out.get("responderState").and_then(|v| v.as_str()),
        Some("available")
    );
    let ids = out
        .get("relayIds")
        .and_then(|v| v.as_array())
        .expect("relayIds array");
...
pub fn ddb_get_relays_status_returns_fail_when_not_allowed() {
    ...
    router2.set_bingle_api(Some(crate::util::mock_bingle_api::to_weak(
        MockApiBoth::new_with_internal_override(Arc::new(InternalStarting)),
    )));
    let responses2 =
