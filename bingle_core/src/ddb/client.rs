...
pub trait DdbClient: Send + Sync {
    ...
    fn registerIP(&self, endpoint: SocketAddr, am_relay: bool) -> Result<(), BingleError> {
        self.register_ip(endpoint, am_relay)
    }
...
impl DdbClientImpl {
    ...
    fn find_relay(&self) -> Result<RelayInfo, BingleError> {
        ...
        let finder =
            crate::relay::relay_finder::RelayFinder::new(self.api.clone(), self.discover.clone());
        tracing::trace!("[DdbClientImpl::find_relay] get_my_id");
        let my_id = self
            .api()?
            .get_my_id()
            .ok_or_else(|| BingleError::Other("get_my_id returned None".to_string()))?;
    ...
    fn relay_user_id(relay_id: &str) -> Result<String, BingleError> {
        ...
        match data_encoding::BASE32_NOPAD.decode(relay_id.as_bytes()) {
            ...
            Ok(bytes) => Err(BingleError::Other(format!(
                "invalid relay id: base32 decoded length {} != 36",
                bytes.len()
            ))),
    ...
    pub fn get_relays_from(
        &self,
        relay: &RelayInfo,
    ) -> Result<Vec<(String, SocketAddr)>, BingleError> {
        ...
        let relay_user = match Self::relay_user_id(relay.id()) {
            ...
            Err(e) => {
                tracing::error!(
                    "[DdbClientImpl::get_relays_from] relay_user_id failed: {}",
                    e
                );
                return Err(e);
            }
        };
        let req = Message::Ddb(DdbMessage::GetRelaysStatus(DdbGetRelaysStatus {
            app: "ddb".into(),
            epoch_id: -1,
            tag: None,
            text: None,
        ...
        let resp = match self.api()?.send_message_to_network_with_response_timeout(
            ...
            Err(e) => {
                tracing::error!(
                    "[DdbClientImpl::get_relays_from] send_message_to_network_with_response failed: {}",
                    e
                );
                return Err(e);
            }
        ...
        if !app_ok || !ty_ok {
            return Err(BingleError::Other(
                "unexpected response to getRelaysStatus".to_string(),
            ));
        }
            ...
            && eps.len() == ids.len()
        {
            ...
            for (i, id) in ids.iter().enumerate() {
                ...
                if let Ok(ip) = host.parse::<IpAddr>() {
                    out.push((id.clone(), SocketAddr::new(ip, port)));
                }
            }
            return Ok(out);
        }
...
impl DdbClient for NullDdbClient {
    fn register_ip(&self, _endpoint: SocketAddr, _am_relay: bool) -> Result<(), BingleError> {
        Err(BingleError::Other(
            "DDB client not configured (missing app_id or unsupported platform)".to_string(),
        ))
    }
    fn register_relay(
        &self,
...
impl DdbClient for DdbClientImpl {
    fn start_load_from_peer(&self, peer_id: &str) -> Result<usize, BingleError> {
        tracing::debug!(
            "[DdbClientImpl::start_load_from_peer][enter] peer_id={}",
            peer_id
        );
        ...
        if nsk_opt.is_none() {
            ...
            match self.lookup(peer_id) {
                ...
                Err(e) => {
                    let err = format!(
                        "failed to resolve peer '{}' via roots or DDB: {}",
                        peer_id, e
                    );
        ...
        let resp: serde_json::Value = match self
            ...
            Err(e) => {
                tracing::error!(
                    "[DdbClientImpl::start_load_from_peer] send_message_to_network_with_response failed: {}",
                    e
                );
                return Err(e);
            }
        ...
        if !app_ok || !ty_ok {
            return Err(BingleError::Other(
                "unexpected response (expected DdbInitResponse)".to_string(),
            ));
        }
    ...
    fn register_ip(&self, endpoint: SocketAddr, am_relay: bool) -> Result<(), BingleError> {
        ...
        let resp = match self.api()?.send_message_to_network_with_response_timeout(
            ...
            Err(e) => {
                tracing::error!(
                    "[DdbClientImpl::register_ip] send_message_to_network_with_response failed: {}",
                    e
                );
                return Err(e);
            }
        ...
        tracing::debug!(
            "[DdbClientImpl::register_ip] received DdbUpdateResponse: {:?}",
            resp
        );
        ...
        if app_ok && ty_ok {
            ...
            Err(BingleError::Other(
                "unexpected response (expected DdbUpdateResponse)".to_string(),
            ))
        }
    }
    ...
    fn signoff(&self) -> Result<(), BingleError> {
        ...
        match self
            .api()?
            .send_message_to_network(&nsk, &relay_user_b64, json, None)
        {
            Ok(true) => Ok(()),
            Ok(false) => Err(BingleError::Other(
                "signoff send returned false".to_string(),
            )),
            Err(e) => {
                tracing::error!(
                    "[DdbClientImpl::signoff] send_message_to_network failed: {}",
                    e
                );
                Err(e)
            }
    ...
    fn register_relay(
        ...
        let relay_user_b64 = match Self::relay_user_id(relay.id()) {
            ...
            Err(e) => {
                tracing::error!(
                    "[DdbClientImpl::register_relay] relay_user_id (root) failed: {}",
                    e
                );
                return Err(e);
            }
        ...
        if let Err(e) = Self::relay_user_id(&relay_id) {
            tracing::error!(
                "[DdbClientImpl::register_relay] relay_user_id (param) failed: {}",
                e
            );
            return Err(e);
        }
        ...
        let resp = match self.api()?.send_message_to_network_with_response_timeout(
            ...
            Err(e) => {
                tracing::error!(
                    "[DdbClientImpl::register_relay] send_message_to_network_with_response failed: {}",
                    e
                );
                return Err(e);
            }
        ...
        tracing::debug!(
            "[DdbClientImpl::register_relay] received DdbUpdateResponse: {:?}",
            resp
        );
