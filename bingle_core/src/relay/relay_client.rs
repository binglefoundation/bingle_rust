...
impl RelayClient {
    ...
    pub fn call(
        &self,
        relay_nsk: &NetworkEndpoint,
        target_id: &str,
    ) -> Result<NetworkEndpoint, BingleError> {
        tracing::info!(
            "[RelayClient::call] my_id={:?}, relay_nsk: {:?}, target_id: {}",
            self.api.upgrade().and_then(|a| a.get_my_id()),
            relay_nsk,
            target_id
        );
        ...
        let relay_id = relay_nsk
            .relay_id()
            .ok_or_else(|| {
                BingleError::Other("RelayClient::call: relay_nsk has no relay_id".to_string())
            })?
            .to_string();
        ...
        let relay_addr: SocketAddr = if let Some(addr) = relay_nsk.relay_address() {
            ...
            resolved.inet_socket_address().ok_or_else(|| {
                BingleError::Other(
                    "RelayClient::call: DDB lookup for relay did not return a direct endpoint"
                        .to_string(),
                )
            })?
        };
        ...
        let channel = channel_opt.ok_or_else(|| {
            BingleError::Other(
                "RelayClient::call: unexpected response (missing channel)".to_string(),
            )
        })?;

        // Return a relay endpoint configured with address + channel + relay id
        Ok(NetworkEndpoint::new_relay(
            relay_id,
            Some(relay_addr),
            Some(channel),
        ))
