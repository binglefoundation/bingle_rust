...
use crate::turn::turn_handler::{
    TurnHandler, TurnMessageWithAddress, TurnRelayHandler, WrappedMessageWithNetworkEndpoint,
};
...
impl TurnRelayHandlerImpl {
    ...
    fn alloc_channel(&self) -> Option<u16> {
        ...
        for _ in 0..range {
            if !(MIN_CH..=MAX_CH).contains(&candidate) {
                candidate = MIN_CH;
            }
...
impl Default for TurnRelayHandlerImpl {
    fn default() -> Self {
        Self::new()
    }
}
...
impl TurnHandler for TurnRelayHandlerImpl {
    fn handle_listen(&self, source_id: &str, source: &SocketAddr) -> bool {
        if let (Ok(mut id2a), Ok(mut a2id)) = (
            ...
            info_theme!(
                themes::TURN,
                "[TurnRelayHandlerImpl::handle_listen] registered {} -> {}",
                source_id,
                source
            );
            true
            ...
            info_theme!(
                themes::TURN,
                "[TurnRelayHandlerImpl::handle_listen] lock poisoned while adding {} -> {}",
                source_id,
                source
            );
            false
    ...
    fn handle_turn_incoming(
        ...
        info_theme!(
            themes::TURN,
            "[TurnRelayHandlerImpl::handle_turn_incoming] {} bytes from {:?}, local_public_address={:?}",
            packet.len(),
            sender_address,
        ...
        let (ch, len, _pad) = match parse_channel_data_header(packet) {
            ...
            None => {
                warn_theme!(
                    themes::TURN,
                    "[TurnRelayHandlerImpl::handle_turn_incoming] failed to parse channel data header ({} bytes)",
                    packet.len()
                );
                return None;
            }
        ...
        let (source_addr, dest_addr) = {
            let map = match self.ch_to_pair.lock() {
                ...
                Err(_) => {
                    tracing::warn!(
                        "[TurnRelayHandlerImpl::handle_turn_incoming] ch_to_pair lock poisoned"
                    );
                    return None;
                }
            };
            if let Some(pair) = map.get(&ch).cloned() {
                tracing::debug!(
                    "[TurnRelayHandlerImpl::handle_turn_incoming] found channel mapping in ch_to_pair for ch {} => {:?}",
                    ch,
                    pair
                );
                pair
                ...
                let sender_addr = match sender_address {
                    ...
                    None => {
                        tracing::warn!(
                            "[TurnRelayHandlerImpl::handle_turn_incoming] no mapping for ch {} and sender_address is None",
                            ch
                        );
                        return None;
                    }
                ...
                let relay_id = match self.allowed_addr_to_id.lock() {
                    Ok(m) => match m.get(&sender_addr).cloned() {
                        ...
                        None => {
                            tracing::warn!(
                                "[TurnRelayHandlerImpl::handle_turn_incoming] no mapping for ch {} and sender {} is not registered",
                                ch,
                                sender_addr
                            );
                            return None;
                    ...
                    Err(_) => {
                        tracing::warn!(
                            "[TurnRelayHandlerImpl::handle_turn_incoming] allowed_addr_to_id lock poisoned"
                        );
                        return None;
                    }
                };
                let payload = match packet.get(4..4 + len) {
                    ...
                    None => {
                        tracing::warn!(
                            "[TurnRelayHandlerImpl::handle_turn_incoming] packet too short for len {} (total {})",
                            len,
                            packet.len()
                        );
                        return None;
                ...
                let network_endpoint =
                    NetworkEndpoint::new_relay(relay_id.clone(), Some(sender_addr), Some(ch));
                tracing::info!(
        ...
        if let Ok(map) = self.allowed_addr_to_id.lock() {
            if !map.contains_key(&dest_addr) {
                tracing::warn!(
                    "[TurnRelayHandlerImpl::handle_turn_incoming] rejecting packet from {}: destination address not registered via Listen",
                    dest_addr
                );
                return None;
            }
            ...
            tracing::warn!(
                "[TurnRelayHandlerImpl::handle_turn_incoming] lock poisoned while checking allowed addr presence"
            );
            return None;
        }
        ...
        if source_addr == dest_addr {
            tracing::info!(
                "[TurnRelayHandlerImpl::handle_turn_incoming] {}: source and destination are the same, indicates we have a message for this relay to receive locally.",
                source_addr
            );
        }
        ...
        let network_endpoint: NetworkEndpoint = if let Some(sender_addr) = sender_address {
            if let Some(relay_id) = self
                ...
                .lock()
                .ok()
                .and_then(|m| m.get(sender_addr).cloned())
            {
                let local_pub = match local_public_address {
                    Some(addr) => addr,
                    None => {
                        tracing::warn!(
                            "[TurnRelayHandlerImpl::handle_turn_incoming] local_public_address is None for relayed message"
                        );
                        return None;
                    }
                };
            ...
            tracing::warn!(
                "[TurnRelayHandlerImpl::handle_turn_incoming] rejecting packet from unknown source {:?}",
                sender_address
            );
            return None;
        };
        ...
        Some(WrappedMessageWithNetworkEndpoint {
            ip_address: if is_packet_from_dest {
                source_addr
            } else {
                dest_addr
            },
...
impl TurnRelayHandler for TurnRelayHandlerImpl {
    fn handle_call(
        ...
        let ch = match self.alloc_channel() {
            Some(v) => v,
            None => return -1,
        };
        // Insert into maps
        if let (Ok(mut c2a), Ok(mut p2c), Ok(mut id2a), Ok(mut a2id)) = (
            ...
            tracing::info!(
                "[TurnRelayHandlerImpl::handle_call] allocated channel {} for {} ({}) -> {} ({})",
                ch,
                source,
                source_id,
                dest,
