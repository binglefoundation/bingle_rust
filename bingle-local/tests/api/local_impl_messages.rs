...
fn add_and_get_single_message() {
    ...
    api.add_message("alice".into(), vec!["bob".into()], 123, "hi".into())
        .expect("add_message");
    let msgs = api.get_messages().expect("get_messages");
...
fn messages_preserve_insertion_order() {
    let mut api = BingleApiLocalImpl::new(LocalApiConfig::default());
    api.add_message("alice".into(), vec!["bob".into()], 1, "m1".into()).unwrap();
    api.add_message("carol".into(), vec!["dave".into(), "erin".into()], 2, "m2".into()).unwrap();
    api.add_message("frank".into(), vec!["george".into()], 3, "m3".into()).unwrap();
...
fn add_message_rejects_empty_inputs() {
    let mut api = BingleApiLocalImpl::new(LocalApiConfig::default());
    // empty sender
    assert!(api.add_message("".into(), vec!["x".into()], 0, "t".into()).is_err());
    // empty recipients
    assert!(api.add_message("s".into(), vec![], 0, "t".into()).is_err());
    // empty recipient in list
    assert!(api.add_message("s".into(), vec!["".into()], 0, "t".into()).is_err());
    // empty text
    assert!(api.add_message("s".into(), vec!["x".into()], 0, "".into()).is_err());
}
...
fn get_messages_returns_clone() {
    ...
    msgs.push(Message { sender_handle: "z".into(), recipient_handles: vec!["y".into()], timestamp: 9, text: "zzz".into() });
