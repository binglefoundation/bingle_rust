...
# androidTest APKs, starts Metro headless, and runs the suite. Detox boots the AVD named in
# .detoxrc.js and adb-reverses Metro's port so the app reaches the host bundler.
#
...
#
# Backend for the network e2e: BINGLE_E2E_BACKEND=testnet (default). localnet on Android needs
# emulator<->host networking (10.0.2.2) and is wired in issue #131.
#
...
# the emulator reaches it. Run Metro headless in the background (reuse one already on 8081).
METRO_BG_PID=""
