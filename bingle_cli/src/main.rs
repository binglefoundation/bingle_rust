...
fn cmd_chat(args: Vec<String>) {
    const USAGE: &str = "Usage: bingle_cli chat [--handle <handle>|<handle>] [--passphrase <text>] [--to <handle> | --to-id <id>] [--state_file <file>] [--node-file <file>] [--app-id <id>] [--asset-id <id>] [--stun-servers <list>] [--stun-servers-file <file>] [--no-retries] [--store-forward <both|send|receive|none>] [--notify <url>] [--info|--debug]\n  --store-forward: enable Sidewinder Mailbox store-and-forward (needs SIDEWINDER_NODE_URL + SIDEWINDER_TOKEN). both=send+receive, send=post on give-up, receive=poll inbox, none=off (default).\n  --notify <url>: nudge the bingle_notify gateway at <url> when a direct send gives up.";
