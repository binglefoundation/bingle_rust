...
pub fn buy_bingle_transfers_from_reserve_inner_tx() {
    ...
    let before = creator.asset_holding(ADDRESS_RECEIVE, asset_id).expect("buyer balance before");
    ...
    let after = creator.asset_holding(ADDRESS_RECEIVE, asset_id).expect("buyer balance after");
