...
#[test]
fn simple_stun_mixed_servers_inconsistent() {
    ...
    let s2 = SimpleStunServer::start(SimpleStunStartOptions { bind_addr: a2, attach_to: None, broken_nat: true }).expect("start s2");
    ...
    let ok = loop {
        if start.elapsed() > Duration::from_secs(3) { break false; }
        {
            ...
            if let Some((st, _)) = rec.iter().rev().find(|(s, _)| *s == StunState::Inconsistent) {
