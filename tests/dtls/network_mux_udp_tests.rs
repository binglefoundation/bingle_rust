...
#[ntest::timeout(30_000)]
#[test]
fn write_relay_wraps_payload_in_turn_channel_data() {
    ...
    assert!(n >= 4 + payload.len(), "wrapped length should include header and payload");
    ...
    for i in 0..padding { assert_eq!(buf[4+len_got+i], 0u8, "padding byte should be zero"); }
}
