...
fn start_pair() -> (Arc<BingleApiImpl>, Arc<BingleApiImpl>, SocketAddr, SocketAddr) {
    ...
    relay.set_id_to_handle_lookup_mock_for_tests(Box::new(|user_id| {
        if user_id.is_empty() { Ok(None) } else { Ok(Some("mock-client".to_string())) }
    }));
    client.set_id_to_handle_lookup_mock_for_tests(Box::new(|user_id| {
        if user_id.is_empty() { Ok(None) } else { Ok(Some("mock-relay".to_string())) }
    }));
...
pub fn ddb_client_signoff_removes_entry() {
    ...
    impl InnerBingleApi for ApiProxy {
        ...
        fn send_message_to_network(&self, nsk: &NetworkEndpoint, uid: &UserId, msg: serde_json::Value, progress: Option<Arc<ProgressCallback>>) -> Result<bool, rust_comms::api::bingle_api::BingleError> {
    ...
    let nsk = cli.lookup(&client_id).expect("lookup should find the freshly registered entry");
    assert_eq!(nsk.inet_socket_address().expect("direct endpoint"), client_addr);
    ...
    while Instant::now() < deadline {
        if cli.lookup(&client_id).is_err() { gone = true; break; }
