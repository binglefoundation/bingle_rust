...
pub fn test_ddb_upsert_record_mandatory_verification() {
    ...
    api.access_unsafe_for_tests(|a| a.start(&opts)).expect("start ok");

    let my_id = api.access_unsafe_for_tests(|a| a.get_my_id()).expect("get_my_id ok");
    let signing_key = api.access_unsafe_for_tests(|a| a.get_signing_key()).expect("get_signing_key ok");
    ...
    let unsigned_record = AdvertRecord::new_unsigned(
        other_id,
        Some(InetSocketAddress { host: "127.0.0.1".into(), port: 1235 }),
        Some(false),
        None,
        None,
        "2025-01-01T00:00:00Z".into()
    );
    assert!(!unsigned_record.verify(), "Unsigned record should not verify");

    // 3. Upsert signed record - should succeed
    api.ddb_upsert_record(signed_record.clone());
    assert_eq!(api.ddb_backend_size(), 1, "Signed record should be upserted");
    ...
    assert_eq!(api.ddb_backend_size(), 1, "Unsigned record should NOT be upserted");
