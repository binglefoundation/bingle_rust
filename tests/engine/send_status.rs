...
use rust_comms::engine::{EndpointStatus, Engine, SEND_FAIL_BACKOFF};
...
impl Dtls for SucceedingDtls {
    fn start(&mut self, _mux: std::sync::Arc<UdpNetworkMux>) -> rust_comms::dtls::Result<()> {
        Ok(())
    }
    ...
    fn get_handle_peer_certificate(&self) -> Option<rust_comms::dtls::HandlePeerCertificate> {
        None
    }
    fn set_handle_peer_certificate(
        &mut self,
        _handler: Option<rust_comms::dtls::HandlePeerCertificate>,
    ) {
    }
    fn with_handle_peer_certificate(self, _handler: rust_comms::dtls::HandlePeerCertificate) -> Self
    where
        Self: Sized,
    {
        self
...
pub fn send_status_is_success_after_successful_send() {
    ...
    let status = status_map
        .get(&key)
        .expect("send_status should contain an entry for the endpoint");
    assert!(
        status.is_working,
        "send_status.success should be true after a successful send"
    );
}
...
pub fn send_status_is_failure_after_failed_send() {
    ...
    let status = status_map
        .get(&key)
        .expect("send_status should contain an entry for the endpoint even on failure");
    assert!(
        !status.is_working,
        "send_status.success should be false after a failed send"
    );
}
...
pub fn send_status_tracks_multiple_endpoints_independently() {
    ...
    assert_eq!(
        status_map.len(),
        2,
        "should have entries for both endpoints"
    );
    assert!(
        status_map
            .get(&key1)
...
pub fn send_status_updates_from_success_to_failure_on_repeated_sends() {
    ...
    assert!(
        engine
            .endpoint_status_for_tests()
            .get(&key)
            .expect("entry after first send")
            .is_working
    );
    ...
    assert!(
        !engine
            .endpoint_status_for_tests()
            .get(&key)
            .expect("entry after second send")
            .is_working
    );
}
...
pub fn send_to_peer_returns_err_immediately_when_recent_failure_recorded() {
    ...
    engine.set_endpoint_status_for_tests(
        key.clone(),
        EndpointStatus {
    ...
    assert!(
        result.is_err(),
        "expected Err due to backoff guard, got {:?}",
        result
    );
...
pub fn send_to_peer_sends_normally_when_failure_is_older_than_backoff() {
    ...
    let old_timestamp = Instant::now()
        .checked_sub(SEND_FAIL_BACKOFF + Duration::from_secs(1))
        .expect("must be able to subtract duration");
    engine.set_endpoint_status_for_tests(
        key.clone(),
        EndpointStatus {
            last_checked_timestamp: old_timestamp,
            is_working: false,
        },
    );
    ...
    assert!(
        result.is_ok(),
        "expected Ok since failure is older than SEND_FAIL_BACKOFF, got {:?}",
        result
    );
    ...
    assert!(
        status.is_working,
        "send_status.success should be true after the successful send"
    );
}
...
pub fn send_to_peer_sends_normally_when_previous_send_succeeded() {
    ...
    assert!(
        result.is_ok(),
        "expected Ok since status is success, got {:?}",
        result
    );
}
...
pub fn packet_arrival_sets_endpoint_status_is_working_true() {
    ...
    let status = status_map
        .get(&key)
        .expect("endpoint_status should have an entry after packet arrival");
    assert!(
        status.is_working,
        "is_working should be true after packet arrival"
    );
    assert!(
