...
struct DummyApi {
    ...
    pub config: Option<rust_comms::blockchain::algo_ops::AlgoChainConfig>,
}
...
impl BingleApi for DummyApi {
    ...
    fn get_app_id(&self) -> Option<u64> { self.app_id }
    fn get_algo_provider_config(&self) -> Option<rust_comms::blockchain::algo_ops::AlgoChainConfig> { self.config.clone() }
    fn get_accounts_cache(&self) -> Option<Arc<std::sync::Mutex<rust_comms::blockchain::algo_bingle::AccountsCache>>> { None }
    ...
    fn send_message_to_id_with_response(&self, _user_id: &UserId, _message: serde_json::Value, _progress: Option<Arc<ProgressCallback>>) -> Result<serde_json::Value, rust_comms::api::bingle_api::BingleError> { Err(rust_comms::api::bingle_api::BingleError::Other("not implemented".into())) }
    fn send_message_to_handle_with_response(&self, _handle: &Handle, _message: serde_json::Value, _progress: Option<Arc<ProgressCallback>>) -> Result<serde_json::Value, rust_comms::api::bingle_api::BingleError> { Err(rust_comms::api::bingle_api::BingleError::Other("not implemented".into())) }
    fn send_message_to_network_with_response(&self, _nsk: &NetworkEndpoint, _user_id: &UserId, _message: serde_json::Value, _progress: Option<Arc<ProgressCallback>>) -> Result<serde_json::Value, rust_comms::api::bingle_api::BingleError> { Err(rust_comms::api::bingle_api::BingleError::Other("not implemented".into())) }
...
pub fn test_ddb_upsert_relay_requires_blockchain_info() {
    let api = DummyApi { app_id: None, config: None };
    let engine = Engine::new(&StartOptions::new("".into()), crate::util::mock_bingle_api::to_weak(api));
    ...
    assert!(found.is_none(), "Relay record should be rejected when blockchain info is missing");
}
...
pub fn test_ddb_upsert_relay_fails_on_blockchain_error() {
    ...
    let config = rust_comms::blockchain::algo_ops::AlgoChainConfig {
    ...
    let api = DummyApi { app_id: Some(123), config: Some(config) };
    let engine = Engine::new(&StartOptions::new("".into()), crate::util::mock_bingle_api::to_weak(api));
    ...
    let found = engine.ddb_backend_lookup_for_tests(&id);
    assert!(found.is_none(), "Relay record should be rejected when blockchain is unreachable (fail-safe)");
}
...
pub fn test_ddb_upsert_non_relay_always_accepted() {
    ...
    let found = engine.ddb_backend_lookup_for_tests(&id);
    assert!(found.is_some(), "Non-relay record should be accepted even without blockchain info");
}
