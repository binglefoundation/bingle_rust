...
use rust_comms::protocol::cert_verify::{dump_ca_public_key_debug, dump_cert_debug};
...
pub fn generate_pki_from_ops_produces_valid_chain_and_expected_cns() {
    ...
    // Construct issuer CN using provided address + ISSUER_SUFFIX
    let issuer_cn = format!("{}{}", address, ISSUER_SUFFIX);

    let (ca_pem, server_cert_pem, _server_key_pem, client_cert_pem, _client_key_pem) =
        generate_pki_from_ops(&ops, &issuer_cn).expect("generate pki");
    ...
    // Dump all generated certificates in human-readable form
    dump_cert_debug("ca_gen", &ca);
    dump_cert_debug("server_gen", &sc);
    dump_cert_debug("client_gen", &cc);
