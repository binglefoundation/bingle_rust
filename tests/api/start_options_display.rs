...
pub fn display_does_not_leak_passphrase() {
    ...
    assert!(!shown.contains("super secret mnemonic words"), "passphrase leaked in Display: {shown}");
    assert!(shown.contains("algo_passphrase: <set>"), "expected masked passphrase marker in: {shown}");
    // Non-sensitive fields should still be visible for debugging.
    assert!(shown.contains("tester"), "handle should be shown in: {shown}");
}
...
pub fn display_shows_none_when_no_passphrase() {
    ...
    let shown = format!("{}", opts);
    assert!(shown.contains("algo_passphrase: None"), "expected None marker in: {shown}");
}
