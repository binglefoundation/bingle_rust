...
impl MockApi {
    fn new(relays: Vec<(String, SocketAddr, RelayState)>) -> (Self, Arc<Mutex<usize>>) {
        ...
        let api = Self { calls: calls.clone(), relays };
        (api, calls)
    }
}
...
impl BingleApi for MockApi {
    ...
    fn send_message_to_network_with_response(&self, _network_source_key: &NetworkEndpoint, _user_id: &UserId, message: JsonValue, _progress: Option<Arc<ProgressCallback>>) -> Result<JsonValue, rust_comms::api::bingle_api::BingleError> {
        ...
        if ty == "getRelaysStatus" && app == "ddb" {
            ...
            let response = DdbRelaysStatusResponse {
                ...
                relay_endpoints: Some(
                    self.relays.iter().map(|(_, addr, _)| InetSocketAddress::from(*addr)).collect()
                ),
            ...
            Err(rust_comms::api::bingle_api::BingleError::Other("unexpected message".to_string()))
        }
    }
...
pub fn relay_finder_picks_available_relay_via_get_relays_status() {
    ...
    let api_arc: Arc<dyn rust_comms::api::bingle_api::BingleApiBoth> = Arc::new(api_raw);
    ...
    assert!(calls >= 1, "at least one GetRelaysStatus call should have been made");
}
