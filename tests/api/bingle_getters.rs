...
pub fn getters_default_none() {
    ...
    assert!(api.access_unsafe_for_tests(|a: &mut BingleApiImpl| a.get_user_id()).is_none());
...
pub fn getters_after_start() {
    ...
    let opts = StartOptions { handle: "tester".into(), ..StartOptions::new("".into()) };
    ...
    assert_eq!(api.access_unsafe_for_tests(|a: &mut BingleApiImpl| a.get_handle()).as_deref(), Some("tester"));
    ...
    assert_eq!(api.access_unsafe_for_tests(|a: &mut BingleApiImpl| a.get_user_id()), my_id);
}
