...
impl InnerBingleApiInternal for MockInternal {
    fn turn_lookup_addr_by_id(&self, id: String) -> Option<std::net::SocketAddr> { self.turn.lookup_addr_by_id(&id) }
    fn turn_handle_listen(&self, id: String, source: std::net::SocketAddr) -> bool { self.turn.handle_listen(&id, &source) }
}
...
pub fn relay_keep_alive_refreshes_mapping_without_response() {
    ...
    assert!(responses.is_empty(), "KeepAlive must not produce a response, got {:?}", responses);
...
pub fn relay_keep_alive_ignored_when_not_a_relay() {
    ...
    assert_eq!(turn.lookup_addr_by_id("ALGOADDR123"), None, "non-relay node must not register a mapping");
}
