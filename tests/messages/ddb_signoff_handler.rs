...
use rust_comms::ddb::{AdvertRecord, InMemoryDdbBackend, DdbBackend};
use rust_comms::messages::handlers::{MessageHandler, DefaultPrintingHandler, FromStruct};
...
use crate::util::reusable_mock_api::{MockApiBoth, InnerBingleApi, InnerBingleApiInternal, to_weak_api_both};
...
impl InnerBingleApiInternal for SignoffCaptureApi {
    fn ripple_message(&self, _message: serde_json::Value, _originator_id: String, _ddb_backend: &dyn DdbBackend) {
...
fn registered_endpoint() -> InetSocketAddress {
    InetSocketAddress { host: "127.0.0.1".to_string(), port: 1234 }
}
...
fn test_on_ddb_signoff() {
    ...
    let inner_api = Arc::new(SignoffCaptureApi { ripple_count: ripple_count.clone(), clear_count: clear_count.clone() });
    ...
    let fresh_backend = || {
        ...
        backend.upsert(AdvertRecord::new_unsigned(alice_id.clone(), Some(registered_endpoint()), Some(true), None, None, "2023-01-01T00:00:00Z".to_string()));
        backend.upsert(AdvertRecord::new_unsigned(bob_id.clone(), Some(registered_endpoint()), Some(true), None, None, "2023-01-01T00:00:00Z".to_string()));
    ...
    // 1. Alice signs off from her registered endpoint (not rippled)
    {
        ...
        assert!(backend.lock().unwrap().lookup(&alice_id).is_none(), "Alice's record should be deleted");
        assert!(from.take_responses().is_empty(), "signoff should not push a response");
        assert_eq!(clear_count.load(Ordering::SeqCst), 1, "relay pool state cache should be cleared");
        assert_eq!(ripple_count.load(Ordering::SeqCst), 1, "signoff should ripple");
    }
    ...
    // 2. Bob tries to sign off Alice's record (start_id != sender)
    {
        ...
        assert!(backend.lock().unwrap().lookup(&alice_id).is_some(), "Alice's record must not be removed by Bob");
        assert_eq!(clear_count.load(Ordering::SeqCst), 0, "no cache clear on unauthorized signoff");
        assert_eq!(ripple_count.load(Ordering::SeqCst), 0, "no ripple on unauthorized signoff");
    }
    ...
    // 3. Alice signs off but from a different endpoint than registered
    {
        ...
        assert!(backend.lock().unwrap().lookup(&alice_id).is_some(), "signoff from wrong endpoint must be rejected");
        assert_eq!(clear_count.load(Ordering::SeqCst), 0, "no cache clear on endpoint mismatch");
        assert_eq!(ripple_count.load(Ordering::SeqCst), 0, "no ripple on endpoint mismatch");
    }
    ...
    // 4. Rippled signoff for Bob (trusted, from a peer relay; not re-rippled)
    {
        ...
        assert!(backend.lock().unwrap().lookup(&bob_id).is_none(), "rippled signoff should delete Bob's record");
        assert_eq!(clear_count.load(Ordering::SeqCst), 1, "rippled signoff should also clear the pool cache");
        assert_eq!(ripple_count.load(Ordering::SeqCst), 0, "an already-rippled signoff must not ripple again");
    }
    ...
    // 5. Not a relay: signoff is ignored entirely
    {
        ...
        assert!(backend.lock().unwrap().lookup(&alice_id).is_some(), "non-relay must not process signoff");
