...
fn test_on_ddb_delete_resolve_auth() {
    ...
    let alice_record = AdvertRecord::new_unsigned(alice_id.clone(), None, None, None, None, "2023-01-01T00:00:00Z".to_string());
    let bob_record = AdvertRecord::new_unsigned(bob_id.clone(), None, None, None, None, "2023-01-01T00:00:00Z".to_string());
    ...
    let inner_api = Arc::new(RippleCaptureApi { ripple_count: ripple_count.clone() });
    ...
    // 1. Alice deletes her own record (NOT rippled)
    {
        ...
        // Should be deleted
        {
            ...
            assert!(lock.lookup(&alice_id).is_none(), "Alice's record should be deleted");
            assert!(lock.lookup(&bob_id).is_some());
        }
        
        // Should have a response
        assert!(!from.take_responses().is_empty(), "Should have sent a response to Alice");
        
        // Should have rippled
        assert_eq!(ripple_count.load(Ordering::SeqCst), 1, "Should have rippled Alice's delete");
    }
    ...
    // 2. Bob tries to delete Alice's record (NOT rippled)
    {
        ...
        {
            let lock = backend_arc.lock().unwrap();
            assert!(lock.lookup(&alice_id).is_some(), "Alice's record should NOT have been deleted by Bob");
        }
        
        // Should not have sent a response (current implementation)
        assert!(from.take_responses().is_empty(), "Should not have sent a response for unauthorized delete");
        
        // Should NOT have rippled
        assert_eq!(ripple_count.load(Ordering::SeqCst), 0, "Should NOT have rippled unauthorized delete");
    }
    ...
    // 3. Rippled delete for Bob's record (from Charlie)
    {
        ...
        {
            let lock = backend_arc.lock().unwrap();
            assert!(lock.lookup(&bob_id).is_none(), "Bob's record should be deleted by rippled message");
        }
        ...
        assert_eq!(ripple_count.load(Ordering::SeqCst), 0, "Should NOT ripple an already rippled message");
    }
}
