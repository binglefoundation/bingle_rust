...
pub fn concurrent_dispatch_responses_should_not_cross_contaminate() {
    ...
    let make_message = |tag: &str| Message::Ping(PingMessage::Ping(PingPing {
        app: "ping".to_string(),
        tag: None,
        response_tag: None,
    ...
    let endpoint_a: std::net::SocketAddr = "127.0.0.1:12101".parse().expect("endpoint should parse");
    let endpoint_b: std::net::SocketAddr = "127.0.0.1:12102".parse().expect("endpoint should parse");
    ...
    router.route_with_network(
        BarrierTaggedHandler { barrier: barrier.clone() },
        &make_message("tag-a"),
        "SENDER_A.ISSUER",
        &nsk_a,
    );
    ...
    let r1 = sent_rx.recv_timeout(Duration::from_secs(3)).expect("expected first concurrent response");
    let r2 = sent_rx.recv_timeout(Duration::from_secs(3)).expect("expected second concurrent response");
    ...
    assert_eq!(uid_a, "SENDER_A.ISSUER", "first response must go to SENDER_A");
    assert_eq!(format!("{}", nsk_got_a), format!("{}", nsk_a), "first response must be sent to endpoint_a");
    assert_eq!(json_a["tag"], serde_json::json!("tag-a"), "first response must carry tag-a");

    assert_eq!(uid_b, "SENDER_B.ISSUER", "second response must go to SENDER_B");
    assert_eq!(format!("{}", nsk_got_b), format!("{}", nsk_b), "second response must be sent to endpoint_b");
    assert_eq!(json_b["tag"], serde_json::json!("tag-b"), "second response must carry tag-b");
