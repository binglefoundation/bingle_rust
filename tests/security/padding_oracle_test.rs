...
fn cbc_ciphers_vulnerable_to_lucky13_are_rejected() {
    ...
    for cipher in cbc_ciphers {
        ...
        if connector_builder.set_cipher_list(cipher).is_err() {
            println!("Skipping cipher {} - not supported by local OpenSSL client", cipher);
            continue;
        }
...
fn aead_ciphers_not_vulnerable_to_lucky13_are_accepted() {
    ...
    for cipher in aead_ciphers {
        ...
        if let Err(e) = res {
            ...
            assert!(!err_msg.contains("no shared cipher"), "AEAD cipher {} should be supported by server", cipher);
            println!("AEAD cipher {} reached handshake, failed later as expected (likely certs): {}", cipher, err_msg);
        } else {
...
fn default_negotiated_cipher_is_aead() {
    ...
    match res {
        Ok(ssl_stream) => {
            ...
            assert!(cipher_name.contains("GCM") || cipher_name.contains("POLY1305"), 
                "Default cipher {} is not an AEAD cipher, potentially vulnerable to Lucky13", cipher_name);
        },
        Err(e) => {
            panic!("Handshake failed with default ciphers even with certificates: {:?}", e);
        }
    }
}
