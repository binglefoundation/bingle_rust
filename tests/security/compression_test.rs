...
fn dtls_configuration_disables_compression() {
    ...
    rust_comms::dtls::dtls_openssl::openssl_impl::configure_dtls12_connector(
    ...
    rust_comms::dtls::dtls_openssl::openssl_impl::configure_dtls12_acceptor(
...
fn dtls_compression_is_disabled_handshake() {
    ...
    let (ca_pem, _srv_crt, _srv_key, cli_crt, cli_key) =
        rust_comms::api::pki::generate_pki_from_ops(&ops).expect("pki gen");
    ...
    match res {
        Ok(_ssl_stream) => {
            ...
            println!(
                "Handshake succeeded, which means the server is at least compatible with our client."
            );
        }
        Err(e) => {
