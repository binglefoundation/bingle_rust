...
use openssl::ssl::{SslMethod, SslConnector, SslVerifyMode, SslVersion, SslOptions};
...
fn dtls_configuration_disables_renegotiation() {
    ...
    assert!(connector_options.contains(SslOptions::NO_RENEGOTIATION), "Client: NO_RENEGOTIATION should be set");
    ...
    assert!(acceptor_options.contains(SslOptions::NO_RENEGOTIATION), "Server: NO_RENEGOTIATION should be set");
    
    println!("Verified that both connector and acceptor configurations explicitly disable renegotiation.");
}
...
fn dtls_handshake_succeeds_with_renegotiation_disabled() {
    ...
    let _ssl_stream = connector.connect("localhost", stream).expect("handshake failed");
